...
class ULSEEvalHelper:
        ...
        for path in tqdm(predictions_paths, desc="Loading predictions"):
            for prediction in torch.load(path):
                ...
                if prediction["modality"] == "text":
                    ...
                    texts_ids.append(prediction["text_idx"])
                    ...
                    labels_ids.append(prediction["label_idx"])

        print(f"Num labels = {len(set(labels_ids))}")
        print(f"Num texts = {len(set(texts_ids))}")
    ...
    def init_index(self, label_predictions, cls):
        ...
        print(f"Added {added} labels in index.")
        index.createIndex()
    ...
    def retrieve(self, index, text_predictions, cls, num_nearest_neighbors):
        ...
        for prediction in tqdm(text_predictions, desc="Searching"):
            ...
            if cls in self.texts_cls[text_idx]:
                ...
                # print(f"retrieved_ids ({len(retrieved_ids)}): {retrieved_ids}")
                # print(f"Num distances ({len(distances)}): {distances}")
                for label_idx, distance in zip(retrieved_ids.tolist(), distances.tolist()):
